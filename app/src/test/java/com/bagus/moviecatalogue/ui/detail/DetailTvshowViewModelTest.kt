...
class DetailTvshowViewModelTest {
    ...
    @Before
    fun setUp() {
        viewModel = DetailTvshowViewModel(moviesRepository)
    }

    @Test
    fun getTvshow() {
        val dummyDetail = Resource.success(DataDummy.getDetailTvshow())
        val tvshow = MutableLiveData<Resource<TvshowEntity>>()
        tvshow.value = dummyDetail

        `when`(moviesRepository.getTvshowDetail(tvshowId)).thenReturn(tvshow)

        val actualValue = viewModel.getTvshowDetail(tvshowId).value?.data
        val expectedValue = tvshow.value?.data
